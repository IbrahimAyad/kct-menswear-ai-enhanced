  ...
  try {
    ...
    for (const vest of vestProducts) {
      try {
        ...
        const { data: existingProducts, error: searchError } = await supabaseAdmin
          .from('products')
          .select('id, name')
        ...
        if (searchError) {
          results.push({ name: vest.name, status: 'error', message: searchError.message })
          errorCount++
          continue
        }
        ...
        if (existingProducts && existingProducts.length > 0) {
          ...
          if (updateError) {
            results.push({ name: vest.name, status: 'error', message: updateError.message })
            errorCount++
          } else {
            ...
            if (imageError) {
              results.push({ name: vest.name, status: 'partial', message: 'Product updated but images failed' })
            } else {
              results.push({ name: vest.name, status: 'success', message: 'Updated successfully' })
              updatedCount++
            }
          }
        } else {
          results.push({ name: vest.name, status: 'not_found', message: 'Product not found in database' })
        }
      } catch (error) {
        results.push({ name: vest.name, status: 'error', message: error instanceof Error ? error.message : 'Unknown error' })
        errorCount++
      }
    }
    ...
    console.error('Error in update-vests API:', error)
    return NextResponse.json({ 
      error: 'Failed to update vest products',
