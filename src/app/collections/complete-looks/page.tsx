...
function mapUnifiedProductToCollectionProduct(unifiedProduct: UnifiedProduct): CollectionProduct {
  return {
    ...
    category: unifiedProduct.category || 'complete-looks',
    subcategory: unifiedProduct.tags?.find(tag => 
      ['starter-bundles', 'professional-bundles', 'executive-bundles', 'wedding-bundles', 'prom-bundles'].includes(tag)
    ) || 'general',
...
function generateCompleteLooksCategories(): CategoryInfo[] {
  ...
  return masterCollection.subCollections.map(sub => ({
    ...
    bgColor: sub.id === 'all-bundles' ? 'from-green-900 to-green-700' : undefined
  }));
}
...
function CompleteLooksContent() {
  ...
  useEffect(() => {
    if (allProducts.length > 0) {
      trackCollectionView('Complete Looks Collection', allProducts);
    }
  ...
  useEffect(() => {
    if (selectedCategory !== 'all-bundles') {
      trackFilterChange('Complete Looks Collection', { category: selectedCategory });
    }
  ...
  const handleQuickView = (product: CollectionProduct) => {
    setSelectedProduct({
      ...product,
      description: product.description || 'Pre-styled outfits and bundles - everything you need in one click'
    });
    setSelectedSize('');
    setQuantity(1);
    ...
    trackProductClick(product, 'Complete Looks Collection');
  };
      ...
      <div className="min-h-screen bg-white">
        <div className="flex items-center justify-center h-96">
          <div className="text-center">
            ...
            <p className="text-gray-600">Loading complete looks collection...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-white">
        <div className="flex items-center justify-center h-96">
          <div className="text-center">
            ...
            <h2 className="text-xl font-semibold mb-2">Unable to load complete looks collection</h2>
...
export default function CompleteLooksCollectionPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-white">
        <div className="flex items-center justify-center h-96">
