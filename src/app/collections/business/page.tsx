...
function mapUnifiedProductToCollectionProduct(unifiedProduct: UnifiedProduct): CollectionProduct {
  return {
    ...
    category: unifiedProduct.category || 'business-casual',
    subcategory: unifiedProduct.tags?.find(tag => 
      ['business-formal', 'business-casual', 'cocktail', 'date-night', 'shirts'].includes(tag)
    ) || 'general',
...
function generateBusinessCategories(): CategoryInfo[] {
  const masterCollection = getMasterCollection('business-casual');
  ...
  return masterCollection.subCollections.map(sub => ({
    ...
    bgColor: sub.id === 'all-business' ? 'from-blue-900 to-blue-700' : undefined
  }));
}
...
function getSmartFilteredProducts(allProducts: CollectionProduct[], selectedCategory: string, masterCollection: any) {
  if (selectedCategory === 'all-business') {
    return allProducts;
  }
...
function BusinessContent() {
  // Fetch products from API using the unified shop hook with business filter
  const masterCollection = getMasterCollection('business-casual');
  const allBusinessCategories = masterCollection?.subCollections
    .find(sub => sub.id === 'all-business')?.filterParams.tags || 
    ['business', 'professional', 'casual', 'smart-casual'];
  
  // Smart filtering: Use both categories and tags for comprehensive product fetching
  const allBusinessDbCategories = masterCollection?.subCollections
    .find(sub => sub.id === 'all-business')?.filterParams.categories || 
    ['Classic 2-Piece Suits', 'Dress Shirts', 'Dress Pants', 'Sport Coats', 'Blazers'];
  ...
  const categories = useMemo(() => {
    return generateBusinessCategories();
  }, []);
  
  // Smart routing for URL-based filtering
  const { currentFilter, updateFilter } = useSmartCollectionRouting('business-casual');
  
  // UI state
  const [selectedCategory, setSelectedCategory] = useState(currentFilter || 'all-business');
  ...
  useEffect(() => {
    setSelectedCategory(currentFilter || 'all-business');
  }, [currentFilter]);
  ...
  useEffect(() => {
    if (allProducts.length > 0) {
      trackCollectionView('Business & Casual Collection', allProducts);
    }
  ...
  const filteredProducts = smartFilterProducts(allProducts, 'business-casual', selectedCategory);
  
  // Track smart filtering
  useEffect(() => {
    if (filteredProducts.length > 0) {
      trackSmartFilter('business-casual', selectedCategory, filteredProducts.length);
    }
  ...
  useEffect(() => {
    if (selectedCategory !== 'all-business') {
      trackFilterChange('Business & Casual Collection', { category: selectedCategory });
    }
  ...
  const handleQuickView = (product: CollectionProduct) => {
    setSelectedProduct({
      ...product,
      description: product.description || 'From boardroom to date night - versatile styles for every occasion'
    });
    setSelectedSize('');
    setQuantity(1);
    ...
    trackProductClick(product, 'Business & Casual Collection');
  };
      ...
      <div className="min-h-screen bg-white">
        <div className="flex items-center justify-center h-96">
          <div className="text-center">
            ...
            <p className="text-gray-600">Loading business & casual collection...</p>
          </div>
        </div>
      </div>
    );
      ...
      <div className="min-h-screen bg-white">
        <div className="flex items-center justify-center h-96">
          <div className="text-center">
            ...
            <h2 className="text-xl font-semibold mb-2">Unable to load business & casual collection</h2>
