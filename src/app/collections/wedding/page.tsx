...
function generateWeddingCategories(): CategoryInfo[] {
  const masterCollection = getMasterCollection('wedding');
  ...
  return masterCollection.subCollections.map(sub => ({
    ...
    bgColor: sub.id === 'all-wedding' ? 'from-rose-900 to-rose-700' : undefined
  }));
}
...
function WeddingContent() {
  // Fetch products from API using the unified shop hook with wedding filter
  const masterCollection = getMasterCollection('wedding');
  ...
  const allWeddingDbCategories = ['Classic 3-Piece Suits', 'Tuxedos', 'Classic 2-Piece Suits', 'Ties', 'Bow Ties'];
  
  const { products: unifiedProducts, loading, error } = useUnifiedShop({
    initialFilters: { 
      categories: allWeddingDbCategories,
  ...
  const categories = useMemo(() => {
    return generateWeddingCategories();
  }, []);
  
  // UI state
  const [selectedCategory, setSelectedCategory] = useState('all-wedding');
  ...
  useEffect(() => {
    if (allProducts.length > 0) {
      trackCollectionView('Wedding Collection', allProducts);
    }
  ...
  const handleQuickView = (product: CollectionProduct) => {
    setSelectedProduct({
      ...product,
      description: product.description || 'Premium quality wedding attire tailored to perfection'
    });
    setSelectedSize('');
    setQuantity(1);
