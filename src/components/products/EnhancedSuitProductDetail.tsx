...
import { ChevronLeft, ChevronRight, Check, Truck, Shield, RefreshCw, Ruler, MessageCircle, Star, Clock, Info, ZoomIn } from 'lucide-react';
...
import OptimizedImage, { PreloadImages } from '../ui/OptimizedImage';
import { preloadGalleryImages } from '@/lib/cloudflare/imageTransform';

interface EnhancedSuitProductDetailProps {
  color: string;
  suitData: {
    productId: string;
    twoPiece: string;
...
export default function EnhancedSuitProductDetail({ color, suitData }: EnhancedSuitProductDetailProps) {
  ...
  const displayImages = useMemo(() => {
    ...
    if (selectedOption === 'twoPiece') {
      ...
      // Prioritize images with vest/3-piece references
      const threePieceImages = allImages.filter(img => 
        img.includes('vest') || 
        img.includes('three') || 
        img.includes('3p')
      );
  ...
  useEffect(() => {
    preloadGalleryImages(displayImages, currentImageIndex);
  }, [currentImageIndex, displayImages]);

  // Reset image index when switching between 2/3 piece
  useEffect(() => {
                ...
                <div className="mt-4 text-sm text-gray-600 space-y-2">
                  ...
                  {selectedOption === 'threePiece' && (
                    <>
                      ...
                      <p>• 5-button vest with welt pockets</p>
                    </>
                  )}
                </div>
              </details>
