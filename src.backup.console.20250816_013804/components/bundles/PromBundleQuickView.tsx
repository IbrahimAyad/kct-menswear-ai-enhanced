...
import { PromBundle } from '@/lib/products/promBundles';
import { useCart } from '@/hooks/useCart';
import { Button } from '@/components/ui/button';

interface PromBundleQuickViewProps {
  bundle: PromBundle;
  onClose: () => void;
}

export default function PromBundleQuickView({ bundle, onClose }: PromBundleQuickViewProps) {
  const [selectedSize, setSelectedSize] = useState('');
  const [isFavorited, setIsFavorited] = useState(false);
  const { addItem } = useCart();

  const sizes = ['34S', '36S', '38S', '40R', '42R', '44R', '46L', '48L'];
            ...
            <div className="space-y-4">
              ...
              <div className="bg-purple-50 rounded-lg p-4 border border-purple-200">
                <div className="flex items-center gap-2 text-purple-700 mb-2">
                  ...
                  <h4 className="font-semibold">Prom Night Tips</h4>
                </div>
                <ul className="text-sm text-gray-700 space-y-1">
                  <li>• Match your date's dress color with accessories</li>
                  <li>• Order 4-6 weeks before prom for best fit</li>
                  <li>• Don't forget dress shoes and cufflinks!</li>
                </ul>
              </div>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-1 text-green-600">
                  ...
                  <span className="text-sm font-medium">
                    {Math.round((bundle.savings / bundle.originalPrice) * 100)}% off prom bundle
                  </span>
                </div>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span>{bundle.suit.color} {bundle.suit.type === 'tuxedo' ? 'Tuxedo' : bundle.suit.type}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Check className="w-5 h-5 text-green-500" />
                    <span>{bundle.shirt.color} Formal {bundle.shirt.fit} Fit Shirt</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <Check className="w-5 h-5 text-green-500" />
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-semibold">Select Tuxedo Size</h3>
              ...
              <div className="border-t pt-4">
                <div className="flex items-center gap-2 text-sm text-gray-600 mb-2">
                  ...
                  <span>Order by {new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toLocaleDateString()} for your prom</span>
                </div>
                <div className="flex items-center gap-2 text-sm text-gray-600">
                  <Clock className="w-4 h-4" />
                  <span>Free rush shipping available</span>
                </div>
              </div>
